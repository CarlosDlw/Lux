...
std::string LuxPipeline::getProjectRoot(const std::string& inputFile) {
    auto parent = fs::path(inputFile).parent_path();
    if (parent.empty()) parent = ".";
    return fs::canonical(parent).string();
}

std::string LuxPipeline::extractNamespace(LuxParser::ProgramContext* tree) {
...
std::unique_ptr<PipelineResult> LuxPipeline::run(const Options& opts) {
    ...
    // ── Step 2: scan ────────────────────────────────────────────────────────
    progress(++step, 5, "scanning .lx files");
    auto allFiles = ProjectScanner::scan(result->projectRoot);
    if (allFiles.empty()) {
        printErrorLine("no .lx files found in '" + result->projectRoot + "'");
