...
void HelpCFormatter::printFunction(const HelpCFunction& fn,
    ...
    // Signature
    printBoxLine(cyan(fn.returnTType) + " " + bold(fn.name) + "(" +
        [&]() {
            std::string args;
            for (size_t i = 0; i < fn.params.size(); i++) {
                if (i > 0) args += ", ";
                args += cyan(fn.params[i].tType) + " " + fn.params[i].name;
            }
            if (fn.isVariadic) {
    ...
    // Returns
    printBoxEmpty();
    printBoxLine(bold("Returns: ") + cyan(fn.returnTType) +
                dim(" (" + fn.returnCType + ")"));
    ...
    printBoxLine(bold("T usage:"));
    {
        ...
        for (size_t i = 0; i < fn.params.size(); i++) {
            ...
            if (fn.params[i].tType == "*char")
                call += "c\"...\"";
            else if (fn.params[i].tType == "int32")
                call += "0";
            else if (fn.params[i].tType == "float32" || fn.params[i].tType == "float64")
                call += "0.0";
            else if (fn.params[i].tType == "bool")
                call += "true";
            else if (fn.params[i].tType.find('*') == 0)
                call += "ptr";
            else
...
void HelpCFormatter::printStruct(const HelpCStruct& s,
    ...
    if (!s.fields.empty()) {
        ...
        for (auto& f : s.fields) {
            ...
            maxTType = std::max(maxTType, f.tType.size());
        }
        maxName = std::max(maxName, (size_t)4);
...
void HelpCFormatter::printTypedef(const HelpCTypedef& td,
    ...
    printBoxLine(bold("T type:    ") + cyan(td.underlyingTType));

    // If underlying is a struct, try to show its fields
    for (auto& s : hdr.structs) {
        if (s.name == td.underlyingTType || s.name == td.name) {
            if (!s.fields.empty()) {
                printBoxEmpty();
...
void HelpCFormatter::printGlobal(const HelpCGlobal& g,
    ...
    printBoxLine(cyan(g.tType) + " " + bold(g.name));
    ...
    printBoxLine(bold("T type: ") + cyan(g.tType));
    printBoxLine(bold("Header: ") + "<" + hdr.headerName + ">");
...
void HelpCFormatter::printRelatedFunctions(const std::string& typeName,
    ...
    for (auto& fn : hdr.functions) {
        ...
        bool uses = false;
        if (fn.returnTType == typeName) uses = true;
        for (auto& p : fn.params) {
            if (p.tType == typeName || p.cType.find(typeName) != std::string::npos)
                uses = true;
        }
