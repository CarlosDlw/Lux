...
This page covers editor configuration for working with T (`.tm`) files. Editor tooling is still in early stages — this page documents what is available today and what is planned.
...
A TextMate grammar for `.tm` files is planned. This will provide:
...
Until then, you can set `.tm` files to use C/C++ syntax highlighting as a rough approximation:

1. Open a `.tm` file in VS Code.
...
A language server for T is planned for the future. It will provide:
...
If you are working on the T compiler itself, the **ANTLR4** VS Code extension is useful for editing the grammar files (`.g4`):
...
You can set up basic filetype detection for `.tm` files:

```vim
" In ~/.config/nvim/filetype.vim or ~/.vim/filetype.vim
au BufRead,BufNewFile *.tm set filetype=c
```
...
```elisp
(add-to-list 'auto-mode-alist '("\\.tm\\'" . c-mode))
```
...
- `grammar/ToLLVMLexer.g4` — Lexer rules (tokens, keywords, operators)
- `grammar/ToLLVMParser.g4` — Parser rules (syntax structure)
